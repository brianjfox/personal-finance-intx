...
describe("fin-host serve stops when told (issue #104)", () => {
  test("SIGTERM ends the process within seconds, even with a client holding a connection open", async () => {
  ...
  test("a host whose parent has gone follows it out", async () => {
    // Stand in for the shell: a process the host is told is its parent.
