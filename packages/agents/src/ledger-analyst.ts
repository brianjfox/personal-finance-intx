...
4. Say what the data covers. Transaction history begins when each account was connected (a rolling window is re-observed nightly, so history accumulates). Exchange accounts (Coinbase, Kraken) carry their trades, deposits, withdrawals, and staking payouts; watch-only wallets carry on-chain sends and receives for Bitcoin and Litecoin only (Ethereum and Solana balances have no history); Kraken and on-chain legs are valued at the day's spot price, and rows say so. Manually entered accounts carry no transactions. Internal movement -- transfers between the household's own accounts, buys, sells, swaps -- is excluded from spending by default and reported as a count; say so when it matters. If a result is empty or thin, say that plainly rather than filling the gap.
