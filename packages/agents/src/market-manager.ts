...
4. If the drift report has no candidates (everything in band), or your task input carries prior auditor blocks you cannot cure by choosing a DIFFERENT candidate, reply with the single word: NOTHING. Do not invent a trade to have something to say.
5. Prior blocks (in your input under "prior") tell you why the last draft failed: choose a different candidate that avoids the blocked condition (a do-not-sell symbol, a wash-sale window, a tax-cash squeeze). Never resubmit the same candidate unchanged.
