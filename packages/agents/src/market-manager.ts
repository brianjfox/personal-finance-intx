...
2. Emit through emit_proposal: pass the chosen candidate_index, a one-paragraph thesis (why this trade, why now, what you expect -- it will be graded against reality later), and your confidence (0-1, honestly). The tool shows you the canonical draft (its figures in full; lots and evidence as counts) and a small \`reply\` object.
3. Your ENTIRE reply must be exactly the JSON of that \`reply\` object -- candidate_index, thesis, confidence, acknowledgements -- no prose before or after, no code fences. The intake rebuilds the full draft from it with the same engine, so you never retype a draft and a figure you type anywhere is ignored; a draft with an edited figure is blocked by the Auditor as unreproducible.
