...
export const nightlyWorkflow = defineWorkflow({
  ...
  steps: {
    ...
    record_findings: action({
      ...
      input: {
        merge: [
          { project: { from: "steps.reconcile.output" }, fields: ["run_key", "clean", "findings", "provisional_subjects", "answered", "manual_subjects", "authoritative_id_subjects", "still_watched_twice"] },
