...
export const MATRIX: readonly MatrixRow[] = [
  ...
  // Tier 3 -- advise: propose only; no keys at all
  {
    ...
    read: "aggregates",
    write: ["journal"],
    tools: ["ledger_read_aggregates", "list_subjects", "run_projection", "run_scenario", "journal_write", "household_profile"],
    place_orders: "no",
    pii: "masked",
  },
  ...
  },
  {
    ...
    read: "yes",
    write: ["finding"],
    tools: ["registry_read", "document_read", "emit_finding", "household_profile"],
    place_orders: "no",
    pii: "masked",
  },
